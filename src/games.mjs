...
Games.get = async function(creatorId, creatorType) {
    const creatorTypeUri = CreatorTypes[creatorType];
    const endpoint = creatorTypeUri === "Users" ? "users" : "groups";

    const games = await filterJSON({
        url: `https://games.roblox.com/v2/${endpoint}/${creatorId}/games?accessFilter=2&limit=50&sortOrder=Asc`,
        exhaust: true,
        filter: (game) => ({
            ID: game.id,
            Name: game.name,
